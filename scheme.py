...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            result = operator.apply_macro(rest,operator.env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target] = expressions.rest.map(lambda x: scheme_eval(x, env)).first
        return target
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    elif expressions.rest is nil or scheme_truep(scheme_eval(expressions.first,env)):
        return scheme_eval(expressions.first,env, True)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        env.bindings[target.first] = MacroProcedure(target.rest, expressions.rest, env)
        return target.first
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        result = original_scheme_eval(thunk.expr, thunk.env)
