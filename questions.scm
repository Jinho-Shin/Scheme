...
(define (enumerate s)
  ...
  (define (add n s) 
    (if (null? s)
      nil
      (cons(cons n (cons (car s) nil)) (add (+ n 1) (cdr s)))
      )
    )
    (add 0 s) 
  )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (define names (car zipped_values))
           (define bindings (cadr zipped_values))
           (append (list (list 'lambda names (let-to-lambda (car body)))) (map (lambda (x) (let-to-lambda x)) bindings))
           ; END PROBLEM 18
           ))
        (else
         ; BEGIN PROBLEM 18
